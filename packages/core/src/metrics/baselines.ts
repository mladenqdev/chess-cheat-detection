...
export interface BaselineTable {
  ...
  /** per-time-class grid points, ascending by rating */
  grid: Partial<Record<TimeClass, RatingGridPoint[]>>;
}
...
/**
 * Interpolates the grid to a player's exact rating, returning the cohort to
 * judge them against. Below/above the measured range it clamps to the nearest
 * endpoint. Returns undefined when the time class has no grid.
 */
export function cohortAt(
  ...
  const pts = table.grid[timeClass];
...
export function compareToCohort(
  ...
  const band = cohortAt(table, player.timeClass, player.rating);
